...
1. Add +r18n-rails+ gem to your <tt>config/environment.rb</tt>:
...
7. Translate models:
   ...
   3. Call +translations+ method in model with all columns to translated:
      ...
      Now model will be have virtual methods +title+, +text+, +title=+ and
      +text=+, which will call +title_ru+ or +title_en+ and etc based on current
      user locales.
