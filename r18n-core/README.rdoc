...
=== Ruby-style Syntax
R18n uses hierarchical, non English-centric YAML format for translations by
default:
...
You can add custom filters for YAML types or any translated string.  Filters
cascade and can communicate with each other.
...
Or Russian has built-in pluralization without any lambdas in YAML:
...
need to create loader objects with 2 methods: +available+ and +load+:
...
locations which will be only used with application translation (useful for plugins).
...
In translation files you can use:
* Strings
...
The translated string has a +locale+ method for determining its locale (Locale instance
...
You can check if the method has a translation:

  t.post.add.translated?   #=> true
  t.not.exists.translated? #=> false

For untranslated strings you can set a default value:
...
You can also add you own filters for translations: escape HTML entries, convert
...
loaded.
  ...
  t.friendship(anne) #=> "She has a friend"
...
* Filter target. YAML type (<tt>!!type</tt>), <tt>String</tt> for all
  translations of <tt>R18n::Untranslated</tt> for absent translations.
...
R18n contains 2 filters to escape HTML entries: by YAML type and global. If you
...
If this isn't secure in your application (for example, user can change
...
non-breaking spaces (for locale, which uses it as digit separator).
...
and pass an instance to <tt>R18n::I18n.new</tt>:
...
error messages in extension directory. R18n contain translations for base words as
extension directory too.

  R18n.extension_places << R18n::Loader::YAML.new('./error_messages/')

== Add Locale

If R18n hasn't got locale file for your language, please add it. It’s very simple:

* Create the file _code_.rb in the locales/ directory for your language and describe locale.
  Just copy from another locale and change the values.
...
* Create in base/, a file _code_.yml for your language and translate the base messages.
  Just copy file from language, which you know, and rewrite values.
* If your language needs some special logic (for example, different pluralization
  or time formatters) you can extend Locale class methods.
* Send a pull request via GitHub (http://github.com/ai/r18n) or just write email with
  the files to me (andrey@sitnik.ru).
