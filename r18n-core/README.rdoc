...
from Markdown syntax, etc.
  ...
  R18n::Filters.add('custom_type', :filter_name) do |content, locale, replace|
...
You can also add global filters for all translated strings:
  ...
  R18n::Filters.add(String, :escape_html) do |content, locale|
    escape_html(content)
  end

You can use lambdas in your translations. If it isn’t secure in your application
(for example, user can change translations), you can disable it:
