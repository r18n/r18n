...
You can add custom filters for some YAML types or any translated strings.
Filters are cascading and can communicate with each other.
...
too, so you can extend or replace them. For example, you can use named variables
...
English locale extends time formatters:
...
You can translate any class, including ORM models:
  ...
  class Product < ActiveRecord::Base
    ...
    # Model has two usual properties: title_en and title_ru
    translations :title
  end
...
Lack of translation in user language doesn't throw an exception for R18n (since
translations to other languages are often done done by enthusiasts, it can be out of date).
R18n will automatically takes next user language (browser sends a list of locales)
and for cultures with two official languages (e.g., exUSSR) it will take second
...
R18n has a framework-agnostic core package and plugins with out-of-box support for
Sinatra, Merb and desktop applications.

== Usage

To add i18n support to your app, use plugin specific for your
environment: <tt>r18n-rails</tt>, <tt>sinatra-r18n</tt> or
<tt>r18n-desktop</tt>.

If you develop you own plugin or want to use only core gem, you have to create
I18n object and assign it to global variable by <tt>R18n.set</tt> or to current thread
...
Translation files use YAML format and are named after en.yml (English) or
en-us.yml (US English variant) with language/country code as per RFC 3066.

In your translations you can use:
* Strings
...
If the locale is not one of user locales, R18n will search it in their sublocales,
or in another locale, which user accepts:

  t.no.in.english #=> "В английском нет"

Translated string has +locale+ method and you can get its locale (Locale instance
or code string if locale is’t supported in R18n):

  i18n.no.in.english.locale #=> Locale ru (Русский)

You can replace some parameters in translated string by passing them as arguments:
  
  name: "My name is %1"

  t.name('John') #=> "My name is John"

Pluralizable messages get item count from the first argument:

  t.robots(0)  #=> "No robots"
  t.robots(1)  #=> "One robot"
  t.robots(50) #=> "50 robots"

If there isn’t pluralization for some number, translation will use +n+. If
there is not a locale file for translation, it will use English pluralization
rules (0, 1 and +n+).

You can check if a message has translation:
...
You can also add you own filter for translations: escape HTML entities, convert
...
When you create filter, pass to <tt>R18n::Filters.add</tt>:
...
You can print numbers and floats according to the rules of user locale:
...
You can add i18n support to any classes, including ORM models:
  ...
  # For example, user knows only Russian
...
All supported locales are stored in R18n gem at +locales+ dir. If you want to
add your locale, please write me to andrey@sitnik.ru.

To get information about locale, instantiate R18n::Locale:
...
You can load translations from any source, not just from YAML files. To load
translation you have to create loader class with 2 methods:
...
If R18n doesn't have locale file for your language, please add it. It’s very simple:
* Create file _code_.rb in locales/ for your language and describe locale.
  You may copy it from another locale and change values.
  * If in your country people mostly know another language (as in many exUSSR
...
* Create in base/ file _code_.yml for your language and translate base messages.
  Just copy the file from a language you know, and rewrite values.
* If your language need special logics (for example, different pluralization
  or time formatters) you can extend Locale class methods.
* Push files to GitHub (http://github.com/ai/r18n) or just write e-mail with
  this files to me (andrey@sitnik.ru).

_Code_ is RFC 3066 code for your language (for example, “en” for English and
“fr-CA” for Canadian French). You can send to my e-mail any questions
(http://sitnik.ru has more contacts of me).
