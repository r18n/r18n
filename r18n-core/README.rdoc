...
=== Ruby-style syntax
R18n by default use hierarchical not English-centrist YAML format for
translations:
...
To access to translation you can call methods with same names:

  i18n.user.edit         #=> "Edit user"
  i18n.user.name('John') #=> "User name is John"
  i18n.user.count(5)     #=> "There are 5 users"

You can use another way to access to translation with keys, which already has
same method in Object class:
...
Translation variables and pluralization (“1 comment”, “5 comments”) is filters
...
You can also translation any class, including ORM models:
...
language (e.g., if translation isn’t available in Belarus R18n will see in
Russian).
