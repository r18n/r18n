...
If translation has has name of Object method you can use another way:
...
too. So you can extend or replace it. For example, you can enable build-in named
variables filter:
...
is very flexible. For example, Russian locale has build-in different
...
language (e.g., if translation isn’t available in Belarusian R18n will see in
Russian):

  i18n = R18n::I18n.new(['be', 'eo'], 'dir/with/translations')
  
  i18n.locales    #=> [Locale be Belarusian, Locale eo Esperanto,
                  #    Locale ru Russian, Locale en English]
  
  i18n.belarusian #=> "Беларуская мова"
  i18n.esperanto  #=> "Esperanto", not in Belarusian, use next user locale
  i18n.russian    #=> "Русский", not in be and eo, use Belarusian sublocale
